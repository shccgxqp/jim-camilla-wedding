...
function buildCssFilter(raw) {
  ...
  for (const { key } of PARAMS) {
    v[key] =
      key === "intensity" || key === "vignette" ? raw[key] : raw[key] * t;
  }
  ...
  if (Math.abs(finalBright - 1) > 0.008)
    parts.push(`brightness(${finalBright.toFixed(3)})`);

  // Contrast
  const contrastVal = 1 + (v.contrast / 100) * 0.8;
  if (Math.abs(contrastVal - 1) > 0.008)
    parts.push(`contrast(${contrastVal.toFixed(2)})`);

  // Saturation
  const saturateVal = Math.max(0, 1 + v.saturate / 100);
  if (Math.abs(saturateVal - 1) > 0.008)
    parts.push(`saturate(${saturateVal.toFixed(2)})`);
      ...
      {open && (
        ...
        >
          <div className="filter-lab-header">
            ...
            <button type="button" className="filter-lab-reset" onClick={reset}>
              重置
            </button>
          </div>
            ...
            {PARAMS.map(({ key, label, hint, min, max, step, def }) => (
              <div key={key} className="filter-lab-item">
                <label className="filter-lab-row">
                  ...
                  />
                  <span
                    className={`filter-lab-val${vals[key] !== def ? " changed" : ""}`}
                  >
                    {vals[key] > 0 && key !== "vignette" && key !== "intensity"
                      ? `+${vals[key]}`
                      : vals[key]}
                  </span>
                </label>
